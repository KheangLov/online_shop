<template>
    <a v-for="notification in notifications" class="dropdown-item" href="#">
        ...
        <span class="ml-1 text-nowrap text-truncate" v-bind:key="notification.id">#{{ notification.data.user_id }}, <strong>{{ notification.data.user_name }}</strong>, {{ notification.data.messages }}</span>
    </a>
</template>
